...
final class URLSessionProvider: ProviderProtocol {
  ...
  private func handleDataResponse<T: Decodable>(data: Data?, response: HTTPURLResponse?, error: Error?, completion: (NetworkResponse<T>) -> ()) {
    ...
    guard let response = response else { return completion(.failure(.serverError)) }

    switch response.statusCode {
    case 200...299:
      guard let data = data, let model = try? JSONDecoder().decode(T.self, from: data) else { return completion(.failure(.serverError)) }
